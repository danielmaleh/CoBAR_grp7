...
def run_cpg_simulation(nmf, cpg_network, preprogrammed_steps, run_time, pbar=True):
    ...
    for _ in range_(int(run_time / nmf.timestep)):
        ...
        for i, leg in enumerate(preprogrammed_steps.legs):
            ...
            # if backwardwalking
            if cpg_network.backward_walking:
                # if leg is 'RF' or 'LF' multiply joint_angles by (0.5,1,1,1,1,1,1), if leg is 'RH' or 'LH' multiply joint_angles by (1.5,1,1,1,1,1,1)
                if leg == 'RF' or leg == 'LF':
                    my_joints_angles = my_joints_angles * np.array([0.5,1,1,1,1,1,1])   
                elif leg == 'RH' or leg == 'LH':
                    my_joints_angles = my_joints_angles * np.array([1.5,1,1,1,1,1,1])
